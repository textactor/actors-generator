...
export class WebsiteTextEnumerator implements TextEnumerator {
  currentId: number;
  constructor(
    private options: { startId: number; endId: number; url: string }
  ) {
    if (!options.url.includes("ID")) {
      throw new Error(`Invalid url: ${options.url}`);
    }
    if (
      !Number.isFinite(options.startId) ||
      !Number.isFinite(options.endId) ||
      options.startId > options.endId
    ) {
  ...
  private async getNext(tries?: number): Promise<string | null> {
    this.currentId++;
    if (
      this.currentId >= this.options.endId ||
      (tries !== undefined && tries > 4)
    ) {
      return null;
    }
    tries = tries || 0;
