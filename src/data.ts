...
import { setLogger } from "@textactor/actor-domain";
...
export const actorNameRepository =
  ActorNameRepositoryBuilder.build(dynamoDbClient);
export const actorRepository = ActorRepositoryBuilder.build(dynamoDbClient);
export const wikiEntityRepository =
...
export async function create() {
  mongoClient = await new MongoClient(DB_CONNECTION).connect();
  const mongoDb = mongoClient.db();

  const conceptContainerRepository = ConceptContainerRepositoryBuilder.build(
    mongoDb,
    new ConceptContainerValidator()
  );
  const conceptRepository = ConceptRepositoryBuilder.build(
    mongoDb,
    new ConceptValidator()
  );
  const conceptWikiEntityRepository = ConceptWikiEntityRepositoryBuilder.build(
    mongoDb,
    new ConceptWikiEntityValidator()
  );
  const wikiSearchNameRepository = WikiSearchNameRepositoryBuilder.build(
    mongoDb,
    new WikiSearchNameValidator()
  );
  const wikiTitleRepository = WikiTitleRepositoryBuilder.build(
    mongoDb,
    new WikiTitleValidator()
  );
